...
### 9-0. OOS 성능 — 모형 복잡도 ↑ 로 정량 비교

![ML 자산가격결정 — 모형 복잡도와 OOS 성능](figures/chB1_ml_sharpe_comparison.png)

> **그림 읽는 법** — 5개 모형의 out-of-sample 결과 (정성적 재현):
> - **(a) 좌 — Sharpe Ratio**: FF6(0.62) → KNS(1.10) → IPCA(1.45) → CAE(1.78) → Deng(1.92).
>   복잡도 ↑ 따라 단조 증가.
> - **(b) 우 — FF6 대비 연간 α(%)**: 같은 순서로 0% → +0.4% → +1.1% → +1.8% → +2.4%.
> - 단, 복잡도 증가는 **transaction cost·overfitting·데이터 요구량 증가** 비용 동반.
>   실무 도입 시 trade-off를 고려해야 한다.
