...
### 5-0. 자산별 샤프비율 — 왜 접점이 최선인가

![왜 접점 포트폴리오가 최선인가 — 샤프비율의 시각적 증거](figures/ch04_sharpe_comparison.png)

> **그림 읽는 법** —
> - **(a) 좌**: 6개 자산(국채·회사채·원자재·REITs·US/EM 주식)의 위험-수익 산점도.
>   접점 포트폴리오(빨간 별)에 그어진 CAL이 가장 가파른 직선.
> - **(b) 우**: 자산별 샤프비율 막대. **접점 포트폴리오(0.65) > 모든 단일 자산(0.28~0.42)**.
> - → 단일 자산 중 가장 좋은 것(US 주식 0.42)을 사는 것보다, 자산을 적절히 섞어 만든 접점 포트폴리오가 더 효율적.
